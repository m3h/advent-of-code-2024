...
def required_presses(required_sequence: Iterable[str], levels: int, level: int = 0):
    ...
    for required_button in required_sequence:
        ...
        optimal_moves = min(required_presses(moves, levels, level+1) for moves in permutations_of_moves)
...
def main():
    ...
    for code in data.splitlines():
        ...
        resolved_sequence = required_presses(code, levels=26)
        ans += resolved_sequence * numeric_code
