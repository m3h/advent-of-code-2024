...
bfs_cache = dict()
def dfs(start, end, mmap):
    cache_key = start, end
    if cache_key in bfs_cache:
        return bfs_cache[cache_key]
    
    neighbours_sorted = get_neighbours(mmap=mmap, n=start, allow_cheats=False, cheat_length=-1)
    neighbours_sorted.sort(key=lambda n: dist(n, end))
